 ...
 * by Jessie Stieger for CSE 373 Assignment 1, Autumn, 2016.
 * Section BE.
...
public class ImageEnhancerWithUndoAndRedo extends Component implements ActionListener {
    ...
    BufferedImageStack undoStack; //Stack to hold actions to possibly undo
    BufferedImageStack redoStack; //Stack to hold undone actions to possibly redo
    ...
    public void actionPerformed(ActionEvent e) {
        //  If the selected option is not undo, save the current image to the undo 
    	// stack before performing that action, and enable the undo button
    	// Also clears the redo stack and disables the redo button if the chosen action
    	// is neither undo or redo
    	
    	BufferedImage biCopy = new BufferedImage(biWorking.getWidth(),biWorking.getHeight(),BufferedImage.TYPE_INT_RGB);
        (biCopy.getGraphics()).drawImage(biWorking,0,0,null);
    	...
    	if (e.getSource() != undoItem){
    		...
    		undoStack.push(biCopy);
    		undoItem.setEnabled(true);
    	}
    	
    	// If the action selected is undo, save the current image to the redo
    	// stack before performing the undo, then performing the undo,
    	// enabling the redo button, and disabling the undo button if the new undoStack is empty.
    	else{
    		redoStack.push(biCopy);
    	...
    	// If the action selected is redo, load the top of the redo stack and disable
    	// the redo button if the stack is now empty.
    	if(e.getSource() == redoItem){
    ...
    public void run() {
        JFrame f = new JFrame("ImageEnhancer WITH Undo AND Redo by Jessie Stieger"); 
    ...
    public void printNumbersOfElementsInBothStacks() {
    	// Prints out the numbers of elements in each of the two stacks (Undo and Redo)
